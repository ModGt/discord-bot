...
exports.run = (client, message, params) => {
    ...
    let guild =  client.dbs.get(message.guild.id);
        ...
        if (!params[0]) {
            ...
            message.channel.sendCode('asciidoc', `= Liste des commandes =\n\n[Utilise ${guild.prefix}help <nomdelacommande> pour plus de details]\n\n${client.commands.filter(c => client.elevation(message.member) >= c.conf.permLevel).map(c => `${guild.prefix}${c.help.name}${' '.repeat(longest - c.help.name.length)} :: ${c.help.description} ${c.conf.enabled ? '' : '[Commande désactivée]'}`).join('\n')}`);
        } else {
            let command = params[0];
            if (client.commands.has(command)) {
                command = client.commands.get(command);
                if (client.elevation(message.member) < command.conf.permLevel) return;
